...
liste = liste + glob.glob('./image/anto/*.png')
liste = liste + glob.glob('image/*.png')
liste = liste + glob.glob('image/laouen/*.png')
liste = liste + glob.glob('imageNew/*.png')
random.shuffle(liste)
print("Nombres d'exemples à traiter : ", len(liste))
file = open("./fileNamePictures.txt","a")

with tf.Session() as sess:
  sess.run(tf.global_variables_initializer())
  saver.restore(sess=sess, save_path=save_path)
  for elm in tqdm(liste):
    ...
    if np.argmax(result) == value and np.max(result) > 0.95:
      cptGest[value][1] += 1
